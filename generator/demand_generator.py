...
class DemandMatrixGenerator:
    ...
    def generate_gravity(self, amount: int) -> list[dict[str, dict[str, int]]]:
        """
        Main function to generate demand matrices
        :param amount: number of demand matrices to generate
        :return: list of demand matrices
        ...
        for node1 in graph_nodes:
            for node2 in graph_nodes:
                if node1 == node2:
                    # demand_matrix_unnorm_base[node1][node2] = 0
                    continue
                ...
                shortest_path = networkx.shortest_path(topology0, node1, node2, weight=lambda u, v, attr: 1)
                ...
                # save demand
                demand_matrix_unnorm_base[node1][node2] = demand_unnorm

        demand_unnorm_sum = sum((
            demand_matrix_unnorm_base[node1][node2] for node1 in graph_nodes for node2 in graph_nodes if node1 != node2
        ))
