...
class BaseHashFunction(metaclass=abc.ABCMeta):
    ...
    def _flow_path(self, topology: networkx.MultiDiGraph,
        ...
        try:
            #print("NEXTHOPS", topology.nodes(), current_node, previous_node, flow.end, flow.start)
    ...
    def run(self, topology: networkx.MultiDiGraph, flows: list[Flow],
                ...
                try:
                    for t in self.route_dct[flow.flow_id]:
                        if link[0] == t[0]:
                            ...
                            if self.old_flow_paths:
                                flow_paths[flow.flow_id] = self.old_flow_paths[flow.flow_id]
                except KeyError:
                    new_route_flows.append(flow)
        ...
        for flow in new_route_flows:
            try:
                ...
                #print("BEFORE FL PATH", topology.nodes(), flow.start, flow.end)
                flow_path = self._flow_path(topology, flow, hash_weights, flow.start, None,
                                            depth=depth, use_flow_memory=use_flow_memory)
                #print("FLOW PATH", flow.flow_id, flow_path)
        ...
        self.old_flow_paths = flow_paths
        #print("FLOW PATHS LAST", flow_paths)
        return flow_paths
